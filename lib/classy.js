...
const classy = (tag, parentStrings, _parentInterpolations) => (
  strings,
  ..._interpolations
) => {
  ...
  const ClassyComponent = props => {
    ...
    const parentInterpolations =
      _parentInterpolations && _parentInterpolations.map(interpolate);
    const interpolations = _interpolations && _interpolations.map(interpolate);
  ...
  ClassyComponent.classy = { tag, strings, interpolations: _interpolations };
